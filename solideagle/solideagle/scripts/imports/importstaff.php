...
class importstaff
{
	...
	private static function newUser($personattr)
	{
		...
		if( strpos(strtolower($personattr->type), "leraar") === false)
		{
			...
			$person->setGroupId(Group::getGroupByName("staffINF")->getId()); //we expect the group staff to exist...
		}else{
			$person->addType(new Type(Type::TYPE_LEERKRACHT));
			$person->setGroupId(Group::getGroupByName("leerkrachtenINF")->getId()); //we expect the group leerkrachten to exist...
		}
