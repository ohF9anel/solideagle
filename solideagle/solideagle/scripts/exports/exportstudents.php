...
class exportstudents
{
	...
	public static function getCSV()
	{
		...
		$sql = "select p.first_name, p.name, p.account_username, gp.name , p.account_password, s.passwordcoaccount1,  s.passwordcoaccount2 
				from allpersons p 
		...
		$csvstring = "Voornaam,Naam,Gebruikersnaam,Wachtwoord,CoAccount1,CoAccount2\n";
		
		$cmd->executeReader()->readAll(function($data) use (&$csvstring)
		{
			$csvstring.= $data->first_name . "," .$data->name . "," .$data->account_username . 
